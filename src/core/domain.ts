...
const avatarSchema = z.object({
  ...
  dataUrl: z
    .string()
    .max(900000)
    .regex(/^data:image\/(png|webp);base64,/),
  frameWidth: z.number().int().min(16).max(256),
  frameHeight: z.number().int().min(16).max(256),
})
export const agentSchema = z.object({
  ...
  model: z.string(),
  policy: z.enum(['supervised', 'draft_only']),
...
export const workspaceSchema = z.object({
  ...
  perRunBudget: z.number().nonnegative(),
...
export const runSchema = z.object({
  ...
  remoteSha: z.string().nullable().optional(),
  hermesStatus: z.string().nullable().optional(),
})
...
export const workflowSchema = z.object({
  ...
  trigger: z.enum(['manual', 'daily', 'weekly']),
  enabled: z.boolean(),
})
...
export const snapshotSchema = z.object({
  ...
  artifacts: z.array(
    z.object({
      ...
      runId: z.string(),
      name: z.string(),
      kind: z.enum(['document', 'storyboard', 'code']),
      content: z.string(),
      createdAt: z.string(),
    }),
  ),
  knowledge: z.array(
    z.object({
      ...
      name: z.string(),
      content: z.string(),
      category: z.enum(['knowledge', 'skill']),
      createdAt: z.string(),
    }),
  ),
  connections: z.array(
    z.object({
      ...
      workspaceId: z.string(),
      toolId: z.string(),
      status: z.enum(['demo', 'connected', 'disconnected']),
    }),
  ),
  ledger: z.array(
    z.object({
...
export type Command =
  ...
  | {
      ...
      workspaceId: string
      runId: string
      action: 'pause' | 'resume' | 'cancel' | 'approve' | 'reject' | 'queue' | 'start'
    }
  | {
      type: 'save-knowledge'
      workspaceId: string
