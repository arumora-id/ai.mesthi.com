...
function fixture() {
  const data = empty(),
    workspace = installPack(data, 'Test studio', 'research')
...
function start() {
  ...
  return {
    ...f,
    data: reduceCommand(f.data, {
      type: 'create-run',
      workspaceId: f.workspace.id,
      title: 'A test mission',
      brief: 'Explore and prepare a draft.',
      workflowId: f.flow.id,
      agentId: f.agent.id,
    }),
  }
}
describe('Workspace and execution boundaries', () => {
  it('installs independent packs without sharing IDs or mutable data', () => {
    const data = empty(),
      one = installPack(data, 'One', 'content'),
      two = installPack(data, 'Two', 'content')
    const first = data.agents.filter((a) => a.workspaceId === one.id),
      second = data.agents.filter((a) => a.workspaceId === two.id)
  ...
  it('rejects cross-workspace agents and run actions without mutating input', () => {
    const f = start(),
      other = installPack(f.data, 'Other', 'software'),
      before = structuredClone(f.data)
    expect(() =>
      reduceCommand(f.data, {
        type: 'create-run',
        workspaceId: other.id,
        agentId: f.agent.id,
        workflowId: f.data.workflows.find((w) => w.workspaceId === other.id)!.id,
        title: 'Wrong scope',
        brief: 'Must fail',
      }),
    ).toThrow('agent in this workspace')
    expect(() =>
      reduceCommand(f.data, {
        type: 'run-action',
        workspaceId: other.id,
        runId: f.data.runs[0].id,
        action: 'cancel',
      }),
    ).toThrow('Run is not in this workspace')
  ...
  it('pauses active runs when a reduced budget cannot cover reservations', () => {
    const f = start()
    const next = reduceCommand(f.data, {
      type: 'update-budget',
      workspaceId: f.workspace.id,
      monthlyBudget: 100,
      dailyBudget: 10,
    ...
    expect(() =>
      reduceCommand(next, {
        type: 'run-action',
        workspaceId: f.workspace.id,
        runId: next.runs[0].id,
        action: 'resume',
      }),
    ).toThrow('per-run budget')
  ...
  it('keeps seeded records within their workspace', () => {
    ...
    for (const run of data.runs)
      expect(data.agents.find((a) => a.id === run.agentId)?.workspaceId).toBe(run.workspaceId)
    for (const artifact of data.artifacts)
      expect(data.runs.find((r) => r.id === artifact.runId)?.workspaceId).toBe(artifact.workspaceId)
  })
})
